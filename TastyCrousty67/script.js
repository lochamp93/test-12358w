...
const LASTSUB_SWAP_MS = 220; // doit matcher --lastsub-swap-ms en CSS

function applyLastSubNameToDom(name) {
  if (!showLastSub) return; // option désactivée via la config
  const n = String(name || "").trim();
  if (!n) return;
  console.log('🔄 Mise à jour dernier sub:', n); // Debug

  document.querySelectorAll('.subLastSubName').forEach(el => {
    const block = el.closest('.subLastSub') || el; // anime tout le bloc (label + pseudo)

    // pas de changement → pas d'anim
    if (el.textContent.trim() === n) return;

    // pas de texte précédent (premier chargement) → pas de fade-out, juste afficher
    if (!el.textContent.trim()) {
      el.textContent = n;
      return;
    }

    block.classList.add('is-updating'); // fade-out + léger glissement (tout le bloc)
    setTimeout(() => {
      el.textContent = n;              // swap pendant l'invisibilité (pas de saut de largeur visible)
      void block.offsetWidth;          // reflow pour relancer proprement la transition
      block.classList.remove('is-updating'); // fade-in à la nouvelle position
    }, LASTSUB_SWAP_MS);
  });
}

// === "Dernier sub" — data (param URL pour init/tests) ===
const lastSubName = GetStringParam("lastSubName", "");
const showLastSub = GetBooleanParam("showLastSub", true);

// Masquer complètement le bloc "dernier sub" si désactivé (retire l'espace réservé aussi)
if (!showLastSub) {
  document.querySelectorAll('.subLastSub').forEach(el => {
    el.style.display = 'none';
  });
}

// Initialiser le titre, la cible et le dernier sub au chargement
applySubGoalLabelToDom(subGoalLabel);
applySubGoalTargetToDom(subgoalTarget);
applyLastSubNameToDom(lastSubName);
...
// écouter les updates des variables globales (quand tes actions Streamer.bot changent subcount / lastSubName)
client.on('Misc.GlobalVariableUpdated', (evt) => {
  try {
    ...
    if (varName === 'lastSubName') {
      applyLastSubNameToDom(v);
    }
  } catch (e) {
    console.warn('[subcount/lastSub] update error', e);
  }
});


// === Mise à jour automatique du "dernier sub" via les events Twitch natifs ===
// Champs confirmés par ton widget chat/alertes (TwitchSub/TwitchResub/TwitchGiftSub) :
// - data.user.name        → pseudo de l'abonné (sub / resub)
// - data.recipient.name   → pseudo du bénéficiaire (gift sub)
client.on('Twitch.Sub', (response) => {
  const name = response?.data?.user?.name;
  if (name) applyLastSubNameToDom(name);
});
...
client.on('Twitch.GiftSub', (response) => {
  // pour un gift sub, on affiche le/la bénéficiaire (recipient), pas le gifter
  const name = response?.data?.recipient?.name;
  if (name) applyLastSubNameToDom(name);
});
