...
// Affichage de l’indicateur de son réactivable
if (showUnmuteIndicator)
	unmuteLabel.style.display = 'inline';

// Texte d’indice pour la zone de saisie "Charger URL"
...
function LoadJSON(settingsJson) {
	fetch(settingsJson)
		...
		.then(data => {
			...
			// Grouper les paramètres par propriété 'group'
...
function LoadSettingsFromStorage() {
	// Récupérer les paramètres de session depuis le local storage
...
// Connexion à Streamer.bot et récupération des actions
...
let client = new StreamerbotClient({
	...
	onConnect: (data) => {
		console.log(`Connexion réussie à Streamer.bot sur ${sbServerAddress}:${sbServerPort}`)
		console.debug(data);
