...
function toAltState(state){
  // ← NOUVEAU : si vibemeter, préparer le ratingBox AVANT (mais invisible)
  if (state === 'vibemeter') {
    const ratingBox = document.getElementById('ratingBox');
    const ratingBoxBg = document.getElementById('ratingBoxBackground');
    const loadingBar = document.getElementById('loadingBar');
    ...
    if (ratingBox) {
      ratingBox.style.backgroundColor = 'rgba(255,0,0,1)';
  ...
  // ← NOUVEAU : ÉTAPE 1 - Remplir la barre d'abord
  const section = pill.querySelector('.pill_subgoal');
  if (section) {
    section.style.setProperty('--value', '0');  // Remplit à 100%
  }

  // ← ÉTAPE 2 - Attendre que la barre soit remplie (300ms) PUIS fade out texte
  setTimeout(() => {
    textbox?.classList.add('is-hidden');
    
    // ← ÉTAPE 3 - Attendre que le texte disparaisse (180ms) PUIS fade out layers + morph
    setTimeout(() => {
      layers?.classList.add('is-hidden');
      
      // Petit délai pour que le fade layers soit visible
      setTimeout(() => {
      ...
      }, 180);  // Délai après fade texte
    }, 180);  // Durée du fade texte (CSS : .2s)
  }, 300);  // Durée du remplissage barre (CSS : .30s)
}
